...
import $ from 'jquery';
import * as Repository from 'block_catalogue/repository';
...
import SELECTORS from 'block_catalogue/selectors';
...
const TEMPLATES = {
    COURSES_CARDS: 'block_catalogue/view-cards',
    COURSES_LIST: 'block_catalogue/view-list',
    COURSES_SUMMARY: 'block_catalogue/view-summary',
...
const getFilterValues = root => {
    ...
    return {
        ...
        category: courseRegion.attr('data-category'),
    };
};
...
const DEFAULT_PAGED_CONTENT_CONFIG = {
    ...
    persistentLimitKey: 'block_catalogue_user_paging_preference'
};
...
const getMyCourses = (filters, limit) => {
    ...
    if (filters.display === 'summary') {
        ...
        summaryDisplayLoaded = true; // TIM
    }
    // return Repository.getEnrolledCoursesByTimeline(params); // TIM
    
    return Repository.getCatalogueCourses(params); // TIM
};
...
const setCourseHiddenState = (courseId, status) => {
    ...
    return setUserPreference(`block_catalogue_hidden_course_${courseId}`, status)
        .catch(Notification.exception);
};
...
//  console.log('in renderCourses, courseData is ', coursesData);
...
//console.log('in pageBuilder',coursesData, currentPage, pageData, actions, activeSearch);
...
//console.log('inside standardFunctionalityCurry', coursesData);
...
// console.log('inside searchFunctionalityCurry', coursesData, currentPage, pageData, actions);
            pageBuilder(coursesData, currentPage, pageData, actions);
            return renderCourses(root, loadedPages[currentPage]);
        }).catch(Notification.exception);
...
const initializePagedContent = (root, promiseFunction, inputValue = null) => {
    ...
    pagedContentPromise.then((html, js) => { //console.log('pagedContentPromise then', html);
...
/**
 * Listen to, and handle events for the catalogue block.
 *
 * @param {Object} root The catalogue block container element.
...
export const init = root => {
    ...
    if (!root.attr('data-init')) {
        ...
        namespace = "block_catalogue_" + root.attr('id') + "_" + Math.random();
