...
import * as View from 'block_catalogue/view';
...
const updatePreferences = (filter, value) => {
  ...
  if (typeof filter === 'undefined') return;
    ...
    // you can only set a type to one of the strings set in lib.php/block_catalogue_user_preferences and only to a value matching the param type defined therein

    let type = null;
    if (filter === 'display') {
        type = 'block_catalogue_user_view_preference';
    } else if (filter === 'sort') {
        type = 'block_catalogue_user_sort_preference';
    } else if (filter === 'customfieldvalue') {
        type = 'block_catalogue_user_grouping_customfieldvalue_preference';
    } else if (filter === 'category') {
        type = 'block_catalogue_user_filter_category';
    } else {
        type = 'block_catalogue_user_grouping_preference';
    }
...
const registerSelector = root => {
    ...
    Selector.on(
        ...
        (e, data) => {
            ...
            pends.push(updatePreferences(filter, pref));

            if (customfieldvalue) {
                root.find(SELECTORS.courseView.region).attr('data-customfieldvalue', customfieldvalue);
                pends.push(updatePreferences('customfieldvalue', customfieldvalue));
            }

            // this forces the updatePreferences to finish saving before re-initialising the view, so the server-work can pick up any changes
            Promise.all(pends).then(()=>{
//console.log('promise all pends');
                // Reset the views.
 ...
 *
 * @param {object} root The root element for the catalogue block
 */
export const init = root => {
    root = $(root);
    registerSelector(root);
   // console.log('export const init inside view_nav');
};
